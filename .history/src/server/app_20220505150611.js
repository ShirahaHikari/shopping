...
const userSumBuyType = new mongoose.Schema({
  type: String,
  count: Number,
})
const UserBuyType = mongoose.model('userBuyType',userBuyType)
const UserSumBuyType = mongoose.model('userSumBuyType',userSumBuyType)
...
app.post('/addUserSumBuyType',async(req,res)=>{
  UserSumBuyType.findOne({type:req.body.type}).then(result=>{
    if(result === null){
      let userSumBuyType = new UserSumBuyType({
        type:req.body.type,
        count:1,
      })
      userSumBuyType.save(function(err){
        if(err){
          res.send('err')
          return;
        }
      ...
      UserSumBuyType.updateOne({type:req.body.type},{$set:{'count':result.count+1}}).then(result=>{
        if(result==null){
          res.send('err')
        }
        res.send('success')
...
app.get('/getUserSumBuyType',async(req,res)=>{
  UserSumBuyType.find().then(result=>{
    if(result == null){
      res.send('err')
      return
    }
