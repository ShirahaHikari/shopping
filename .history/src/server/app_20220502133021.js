...
app.post('/addUserBuyType',async(req,res)=>{
  UserBuyType.find({name:req.body.name,type:req.body.type}).then(result=>{
    if(result === null){
      ...
      UserBuyType.updateOne({name:req.body.name,type:req.body.type},{$set:{'count':result.count+1}})
    }
  })
})
...
app.get('/getUserReceived', async (req, res) => {
  UserBuyRecord.find().then(result=>{
    // console.log(result)
    if(result==null){
      res.send('failed')
