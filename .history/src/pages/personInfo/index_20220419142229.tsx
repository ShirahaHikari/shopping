...
class PersonalInfo extends React.Component<any,any> {
    ...
    componentDidMount() {
        ...
        axios.post('http://localhost:3000/getUserShop', userData).then((res) => {
            if (res.status === 200) {
                this.setState({ userShop: res.data[0] })
                console.log(res.data[0])
                console.log(this.state.userShop)
            }
        }).catch((err) => {
    ...
    render() {
        return (
            <div>
                <div className={styles.menu} id='top'>
                    <Menu selectedKeys={['mail']} mode="horizontal" >
                        ...
                        <Menu.Item key='hello' disabled style={{ width: 150, left: 120 }}>
                            你好！{this.state?.user?.username ? this.state?.user?.username : ''}
                        </Menu.Item>
                    </Menu>
                </div>
                <Button onClick={() => {
                ...
                <Table id='tableData' columns={this.state.columns} dataSource={this.state.dataShop}></Table>
            </div >
        )
    }
}
