...
import { Menu, Button, message, Modal, Carousel, Table, Form, Input, Card } from 'antd'
import {
    AppstoreOutlined,
    MenuUnfoldOutlined,
    SettingOutlined,
        ...
        this.state = {
            ...
            columnsBuyRecord: [
                ...
                },
                {
                    ...
                    dataIndex: 'price',
                    key: 'price',
                    sorter:(a:any,b:any)=>a.priceSum - b.priceSum,
                    render: (record: any) =>
                        <div style={{ width: '50px', margin: '0 auto' }}>{record}</div>
                },
                {
                    ...
                    render: (text: any, record: any) => {
                        if (record.received === false) {
                            return <a onClick={() => {
                                ...
                                const userData = { name: user.username }
                                axios.post('http://localhost:3000/updateUserReceived', params).then((res) => {
                                    if (res.status === 200) {
                                        message.success('状态修改成功！')
                                        axios.post('http://localhost:3000/getUserReceived', userData).then((tmp) => {
                                            if (tmp.status === 200) {
                                                let res1 = tmp.data[0].map((index: any) => {
                                                    let tmp = {
                                                        item: index.item,
                                                        count: index.count,
                                                        price: index.price,
                            ...
                            <a onClick={() => {
                                ...
                                axios.post('http://localhost:3000/findUserShop', params).then((tmp: any) => {
                                    if (tmp.status === 200) {
                                        ...
                                        axios.post('http://localhost:3000/addUserBuyType', eCharts).then((res1) => {
                                        ...
                                        axios.post('http://localhost:3000/addUserBuyRecord', params).then((res1) => {
                                            if (res1.status === 200) {
                                                ...
                                                const userData = { name: user.username }
                                                axios.post('http://localhost:3000/getUserBuyRecord', userData).then((res3) => {
                                                    if (res3.status === 200) {
                                                ...
                                                axios.post('http://localhost:3000/getUserBuyType', userData).then((res) => {
                                                    if (res.status === 200) {
                                                    ...
                                                    message.error('因服务器原因无法获得用户购买分类！')
                                                })
                                                ...
                                                axios.post('http://localhost:3000/getUserReceived', userData).then((res3) => {
                                                    if (res3.status === 200) {
                                                        let res4 = res3.data[0].map((index: any) => {
                                                            let tmp = {
                                                                item: index.item,
            ...
            <div style={{ width: '100%', height: '1000px' }}>
                <div className={styles.floatButton}>
                    ...
                    {(this.state.buttonVisible && <a onClick={() => {
                        ...
                        echart?.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
                    }}><div style={{ width: '80px' }}>数据图表</div></a>)}
                    {(this.state.buttonVisible && <a onClick={() => {
