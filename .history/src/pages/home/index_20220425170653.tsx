...
const HomePage = (props: any) => {
    ...
    let [userAdvise, setAdvise] = useState([])
    let [formFlag, setFlag] = useState(false)
    const columns = [{
        title: '用户名称',
        dataIndex: 'name',
        key: 'name',
    ...
    const onFinish = (values: any) => {
        ...
        let sendDate = year + ' 年 ' + month + ' 月 ' + day + ' 日 '
        let send = {
            name: user?.username,
            date: sendDate,
            advise: values.advise,
        }
        axios.post('http://localhost:3000/addUserAdvise', send).then(function (res) {
            if (res.status === 200) {
                if (res.data === 'success') {
                    message.success('建议添加成功')
                    setFlag(false)
                } else {
                    message.error('因服务器原因建议添加失败')
                    return
                }
            }
        }).catch((err) => {
            message.error('因服务器原因添加失败')
        })
    }
    ...
    return (
        <div style={{ backgroundColor: 'rgb(234,232,235)', width: '100%', height: '3500px' }}>
            <div className={styles.floatButton}>
                ...
                <a onClick={() => {
                    ...
                    advise?.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
                }}><div style={{ width: '40px' }}>用户评论</div></a>
            </div>
            ...
            <div className={styles.secondPage}>
                <div id='advise'>
                    <h2 style={{ marginTop: 30, marginLeft: '60px' }}>用户评论与建议</h2>
                    {formFlag && <Form
                        name="basic"
                        labelCol={{ span: 6 }}
                        wrapperCol={{ span: 10 }}
                    ...
                    >
                        <Form.Item
                            label="你的建议"
                            name="advise"
                            rules={[{ required: true, message: '请输入你的建议！' }]}
                        >
                            <Input />
                        </Form.Item>
                    ...
                    <Table columns={columns} dataSource={[userAdvise]}></Table>
                </div>
            </div>
        </div>
    )
