...
import notebookComputer from '..';

let userInfo = window.sessionStorage.getItem('userInfo')
console.log(userInfo)
const { SubMenu } = Menu
const NotebookComputer1 = () => {
    let user
    if (userInfo == null) {
        message.error('你没有登录！')
        setTimeout(() => {
    ...
    let userShop = {
        ...
        item: '【爆款】asus华硕天选2游戏本11代英特尔酷睿i7/i5/RTX3050Ti笔记本电脑大学生电竞吃鸡商务办公便携15.6英寸',
        priceSum: Math.floor(count * 6099 * 1000) / 1000,
        count: count,
        price: 6099,
        img: 'https://g-search1.alicdn.com/img/bao/uploaded/i4/imgextra/i2/26623201/O1CN01f15JA21ZW6YYDwSjp_!!0-saturn_solar.jpg_580x580Q90.jpg_.webp',
        type: '笔记本电脑',
    }
    function add() {
        setCount(count + 1)
    ...
    function buy() {
        Modal.confirm({
            title: `确定要直接购买吗,价格${Math.floor(count * 6099 * 1000) / 1000}元`,
            content: '',
            okText: '确认',
            cancelText: '取消',
            onOk: () => {
                ...
                axios.post('http://localhost:3000/addUserSumBuyType', eSumCharts).then((res1) => {
                    if (res1.status === 200) { } else {
                        message.error('分类加载错误')
                    }
                })
                axios.post('http://localhost:3000/addUserBuyType', params).then(function (res) {
                    if (res.status === 200) {
                        message.success('购买成功！')
                    }
                }).catch((err) => {
                    message.error('因服务器原因购买失败！')
                })
                axios.post('http://localhost:3000/addUserBuyRecord', params1).then(function (res) {
                    if (res.status === 200) {
                        // message.success('购买成功！')
                    }
                }).catch((err) => {
        ...
        <div style={{ backgroundColor: 'rgb(234,232,235)', width: '100%', height: '600px' }}>
            ...
            <div className={styles.itemDetail}>
                <div className={styles.picLeft}>
                    <img src='https://g-search1.alicdn.com/img/bao/uploaded/i4/imgextra/i2/26623201/O1CN01f15JA21ZW6YYDwSjp_!!0-saturn_solar.jpg_580x580Q90.jpg_.webp' style={{ width: 300, height: 300 }} alt="" />
                </div>
                <div className={styles.wordRight}>
                    <h2 style={{ marginTop: 30 }}>【爆款】asus华硕天选2游戏本11代英特尔酷睿i7/i5/RTX3050Ti笔记本电脑大学生电竞吃鸡商务办公便携15.6英寸</h2>
                    <div style={{ backgroundColor: 'rgb(255,242,232' }}>
                        <span>价格   </span>
                        <span style={{ color: 'rgb(255,68,0)', fontSize: '30px', marginLeft: 23, fontWeight: 'bold' }}>￥6099.00</span>
                    </div>
...
export default NotebookComputer1;